...
begin
 ...
 if operation = 'INSERT' then
    -- TODO: проверка на то что таблица указана в формате shaema.table, иначе ошибка
        ...
        FROM regexp_matches(trim(substr(Query,ss_util.instr(Query_ucase,'INTO ',1) + 4)),
        '\.([[:graph:]]+)') AS t;
...
-----------------------DELETE------------------------
    ...
    begin
      ...
      where c.relname = cur_table_name  AND a.attnum > 0 AND n.oid = c.relnamespace AND a.attrelid = c.oid and n.nspname = cur_scheme_name
